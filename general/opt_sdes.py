...
def USAM_SDE1(nit, eta, rho, dt,problem):
  ...
  for k in tqdm(range(nit_sde-1)):
    ...
    x_it = x_it - dt*grad_tilde + jnp.array(np.sqrt(eta)*(np.eye(d)+rho*hess_local)@Sigma@delta_W) # This is USAM: Original Antonio Version
...
def SAM_SDE1(nit, eta, rho, dt,problem):
  ...
  for k in tqdm(range(nit_sde-1)):
    ...
    x_it = x_it - dt*grad_tilde + jnp.array(np.sqrt(eta)*(np.eye(d)+(rho/jnp.linalg.norm(grad_local))*hess_local)@Sigma@delta_W) # This is USAM: Original Antonio Version
